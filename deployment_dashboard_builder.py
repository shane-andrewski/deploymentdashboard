...
class DashboardBuilder(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Button(top, text="Preview HTML", command=self.preview_html, style="Tech.TButton").grid(row=0, column=8, padx=(4, 0))
...
<div class="header">
  ...
  <div class="actions">
    ...
    <span class="pill">Local browser edits persist on this device</span>
  </div>
</div>
<div class="layout">
...
function renderExports(loc) {{
  ...
  if (!loc.exports || !loc.exports.length) {{
    exportsEl.innerHTML = '<div class="note">No attached export paths yet. In the local Python builder, attach your generated HTML files. On GitHub Pages, these need to be published with the site to open for others.</div>';
    return;
  }}
